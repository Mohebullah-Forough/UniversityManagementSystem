...
<div class="fluid">
	<div class="row bg-dark bg-gradient">
		...
		<div class="col-8 my-2 mx-auto text-white">
			<p class="text-center fs-3">Edit Events</p>
			<hr class="border border-warning border-2 opacity-50">
			<form action="<%=request.getContextPath()%>/staffs/updateEvent" method="POST">
			
			<input type="hidden" name="eventId" class="form-control"
							id="eventId" value="${event.eventId}" required>
							
				<div class="row mb-3">
					<div class="col-md-6">
						<label for="eventTitle" class="form-label">Event Title</label> <input
							type="text" name="eventTitle" class="form-control"
							id="eventTitle" value="${event.eventTitle}" required>
					</div>
					<div class="col-md-6">
						<label for="eventDate" class="form-label">Event Date</label> <input
							type="date" name="eventDate" class="form-control" id="eventDate" value="${event.eventDate}"
							required>
					</div>
				</div>
				...
				<div class="mb-3">
					...
					<textarea name="eventDescription" class="form-control" placeholder="Write your event's description here..."
						id="eventDescription"  style="height: 100px">${event.eventDescription}</textarea>
				</div>
				...
				<div class="mb-3">
					<label for="eventDepartment" class="form-label">Event Department</label> <select
						...
						<option value="${event.eventDepartment}" selected >${event.eventDepartment}</option>
				...
				<div class="md-3">
						<label for="eventLocation" class="form-label">Event Location</label> <input
							type="text" name="eventLocation" class="form-control" value="${event.eventLocation}"
							id="eventLocation" required>
					</div>
</br>
				<button type="submit" class="btn btn-warning form-control">Edit
					Event</button>
			</form>
